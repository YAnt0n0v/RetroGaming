...
class NextBrick: UIView {
    ...
    func makeGameButton() {
        ...
        self.gameButton.addTarget(self, action: #selector(NextBrick.changeGameState(_:)), for: .touchUpInside)
        self.addSubview(self.gameButton)
        
        self.stopButton.translatesAutoresizingMaskIntoConstraints = false
        self.stopButton.addTarget(self, action: #selector(NextBrick.gameStop(_:)), for: .touchUpInside)
